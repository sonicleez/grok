...
- Frontend calls Ollama Cloud through `'/api/ollama-chat-completions'` (see [src/services/api.js](src/services/api.js)); this assumes a serverless/API proxy exists in deployment. Keep this contract stable unless proxy routing is intentionally changed.
